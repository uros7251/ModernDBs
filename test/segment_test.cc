...
TEST_F(SegmentTest, FSIPersistence) {
   ...
   schema_segment.set_schema(getTPCHSchemaLight());
   auto& table = schema_segment.get_schema()->tables[0];
   table.allocated_pages = 2;
   
   uint64_t freePage;
   {
      FSISegment fsi_segment(table.fsi_segment, buffer_manager, table);
      fsi_segment.update(0, 0);
      fsi_segment.update(1, 64);
      auto page = fsi_segment.find(42);
      ASSERT_TRUE(page);
      freePage = *page;
      ASSERT_EQ(freePage, 1);
   } // Tear down FSI. The free-space info should be stored in the buffer manager
   {
      // A fresh FSI should read its info from buffer pages
      FSISegment fsi_segment(table.fsi_segment, buffer_manager, table);
      auto page = fsi_segment.find(42);
      ASSERT_TRUE(page);
      ASSERT_EQ(freePage, *page);
   }
}
...
TEST_F(SegmentTest, SPRecordWriteRead) {
    ...
    for (uint64_t size = 1; size < max; size *= 2, ++i) {
        auto tid = tids[i];
        std::memset(writeBuffer.data(), static_cast<int>(size & 0xFF), size);
        sp_segment.read(tid, readBuffer.data(), size);
        ASSERT_EQ(std::memcmp(writeBuffer.data(), readBuffer.data(), size), 0);
    }
}
